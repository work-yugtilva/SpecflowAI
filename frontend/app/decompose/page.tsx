...
function adaptPipelineResult(data: Record<string, unknown>): Decomposition {
  ...
  // New flat array format: each element has a "layer" field (ui | backend | system)
  const isFlat =
    allDecomps.length > 0 &&
    allDecomps[0] != null &&
    typeof allDecomps[0] === "object" &&
    "layer" in allDecomps[0];

  if (isFlat) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const flat: any[] = allDecomps;
    const uiComponents: UIComponent[] = flat
      .filter((c) => c.layer === "ui")
      .map((c, i) => ({
        id: c.id ?? `ui${i}`,
        name: c.title,
    ...
    const dataEntities: DataEntity[] = flat
    ...
    const workflowSteps: WorkflowStep[] = flat
      .filter((c) => c.layer === "backend")
      .map((c, i) => ({
        step: i + 1,
        title: c.title,
        description: c.description ?? "",
        actor: "API" as WorkflowStep["actor"],
        outputs: [],
      }));
    return {
      featureTitle: features[0]?.title ?? features[0]?.name ?? "Generated Feature",
  ...
  // Legacy nested-object format: component keys are dynamic names generated by the AI.
